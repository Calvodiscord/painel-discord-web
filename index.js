...
// ROTA: Fornece a lista de membros do servidor para o painel
...
// ROTA: Processa a punição vinda do painel
app.post('/api/punir', async (req, res) => {
    try {
        ...
        const guild = await client.guilds.fetch(process.env.guildId);
        const punishmentChannel = await guild.channels.fetch(process.env.punishment_channel_id); // Corrigido para buscar canal da guilda
