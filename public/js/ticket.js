document.addEventListener('DOMContentLoaded', () => {
    ...
    addLinkBtn.addEventListener('click', () => {
        ...
        if (!url) return;
        const linkText = replyInput.value.substring(replyInput.selectionStart, replyInput.selectionEnd) || "Clique aqui";
        const markdownLink = `[${linkText}](${url})`;
        
        const start = replyInput.selectionStart;
        const end = replyInput.selectionEnd;
        replyInput.value = replyInput.value.substring(0, start) + markdownLink + replyInput.value.substring(end);
        replyInput.focus();
    });
